...
To keep watching your test suites that you are working on, just set `singleRun: false` in the `karma.conf.js` file. Please be sure to set `CONTINUOUS_INTEGRATION: true` as an env variable if you are running `npm test` on a continuous integration server (travis-ci, etc).
